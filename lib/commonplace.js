...
Commonplace.Layer = function(div, params) {
  ...
  this.select_change_event = function(el) {
  	var d, data, o, val;
  	self.actual_element = $(el).attr("id");
  	val = $(el).children(":selected").html();

	
	if (self.communication) {
		o = { type: 'change', element: self.active, val: val };
		self.event = o;
		data        = {};
        data.event      = o;
        data.func       = "window.collaborator.select_interaction";
  ...
  this.select_focus_event = function select_focus_event() {
	var d, data, o;

	if (self.actual_element) {
		o = { type: 'focus', element: self.active };
		self.event = o;
		
		// now the event get transmitted voa socket.io
        if (self.communication) {
        	console.log("SELECT FOCUS WIRD ABGESCHICKT");
 ...
 this.disconnect = function() {
  ...
  this.paint_toolbar = function() {
	...
	this.interaction = function interaction(data) {
	...
	this.select_interaction = function select_interaction(data) {
		$(data.event.element).val(data.event.val);
		
		console.log(data.event.elemnt +" " + data.event.val)
		
	};


	this.text_interaction = function text_interaction(data) {
    ...
    alert("Das ist der Body");
       ...
       $(name).mousemove(function(event){
		...
		$("select").focus(function(){
			// self.select_focus_event(this);
		});	  

	   
		$("select").change(function(){
			self.select_change_event(this);
		});  
